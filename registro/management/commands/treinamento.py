...
class Command(BaseCommand):
    ...
    def treinamento_face(self):
        ...
        for coleta in ColetaFaces.objects.all():
            ...
            # Melhor pré-processamento das imagens
            imagemFace = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            imagemFace = cv2.resize(imagemFace, (220, 220))

            # Equalização do histograma para melhorar contraste
            imagemFace = cv2.equalizeHist(imagemFace)

            # Normalização da imagem
            imagemFace = cv2.normalize(
                imagemFace, None, 0, 255, cv2.NORM_MINMAX)
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(
                f"Imagens com erro no carregamento: {erro_count}"))
